...
describe FindNumber do
  ...
  describe '#game_over?' do
    ...
    context 'when guess and random_number are not equal' do
      let(:number_guessing) { double('number_guessing', value: 3) }
      subject(:new_game_guessing) { described_class.new(0, 9, number_guessing, 5) }
      it 'is not game over' do
        expect(new_game_guessing.game_over?).to be false
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        ...
        expect(guess_game.min).to eql(guess_game.max)
      end

      it 'does not update max' do
        guess_game.update_range
        expect(guess_game.max).to eql(8)
      end
    end
  end
end
