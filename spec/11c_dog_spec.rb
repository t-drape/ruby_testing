...
describe Dog do
  ...
  subject(:my_dog) { described_class.new('RICO', 'doberman', 'black') }

  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'Dog talks because it is child class of Pet' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(my_dog.barking?).to be false
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(my_dog.sleeping?).to be true
  end
end
